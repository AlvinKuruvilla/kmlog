#!/bin/bash
# NOTE: This code is taken directly from the slog repository with a slight modification to 
# no longer print dates
#--------------------------------------------------------------------------------------------------
...
log() {
    ...
    if [ $log_level_stdout -le $log_level_int ]; then
        # STDOUT
        printf "${log_color} [${log_level}] ${log_text} ${LOG_DEFAULT_COLOR}\n";
    fi
